...
We want to learn about the clusters by considering the correlations between them and other features of users and movies. Figures \ref{fig:explore.cluster.genre} and \ref{fig:explore.cluster.occupation} represent movie cluster frequency per movie genre and user cluster frequency per occupation, respectively. As we can see in \ref{fig:explore.cluster.genre}, the first movie cluster is highly correlated to Documentary, Fantasy, and Mystery genre; meanwhile the second corresponds to Western; the third one is Crime and Film-Noir; the last one is about Animation and Musical. 
\begin{figure}[t!]
	\centering
...
Similarly, for users clusters, it can be seen that the first cluster is mainly about doctors; the second cluster is corresponding to retired users and a part of art and health care; the last cluster contains most of the rest occupations. We can also notice that the proportion of users doing marketing belong to all there clusters equally, and almost no doctor belongs to cluster 3.

We also plot the average ratings of each pair of (movies cluster, users cluster). It appears that movies of cluster 4 (Animation and Musical) are liked by user cluster 3 (users of most of the occupations except doctors and retied); movies of cluster 3 (Crime, Film-Noir, and Mystery) have a high average rating by all users; users of cluster 1 (mostly doctors) only watch movies from cluster 2 (Documentary, Western) and 3 (Crime and Film-Noir), and prefer movies from the latter cluster.
