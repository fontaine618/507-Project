...
%------------------------------------------------------------------------------
\subsection{Restricted Boltzmann Machine}\label{sebsec:results.rbm}
...
\begin{figure}[t!]
    \centering
	\includegraphics[scale=0.6]{rating_diff_per_age_gender_genre_9}
...
\Cref{fig:explore.cluster.movies,fig:explore.cluster.users} contain scatter plots of these embeddings: clustering patterns can be seen clearly. Using the $K$-means algorithm\footnote{Using the \texttt{KMeans} class in \texttt{sklearn.cluster} \citep{scikit-learn}.}, we identify 4 movie clusters and 3 user clusters.
...
Experimenting with various approaches to train effective predictive models for user ratings of movies allowed us to identify that matrix completion using a singular value decomposition exhibited the best performance out of all models considered. This simple, fast and interpretable technique has lower testing prediction error and higher testing accuracy than $K$-nearest-neighbors, neural networks and restricted Boltzmann machine. Interestingly, it is able to produce better results than its competitor even though no external information is included in the model. 
...
%------------------------------------------------------------------------------
\section{Derivations for Restricted Boltzmann Machine}\label{sec:rbm}
