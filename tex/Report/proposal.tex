...
As was mentionned in the description of the $K$-NN model (\Cref{subsubsec:method.models.knn}), our discrete numerical response can be treated in multiple ways: for this NN approach we consider five such treatments.
...
\caption{Neural network architecture described in \Cref{subsubsec:method.models.nn}. Black nodes represent observed quantities; white nodes, hidden variables; grey squares, functions.\label{fig:method.models.nn}}
\end{figure}
...
\begin{figure}
	...
	\caption{Restricted Boltzmann Machine in \Cref{subsubsec:method.models.rbm}. Each user has a graphical model connecting hidden variables and visible ratings. Weight $W_{ij}^{k}$ shares among users.\label{fig:method.models.rbm}}
\end{figure}
...
Upon training and testing 35 instances of the $K$-NN model described in \Cref{subsubsec:method.models.knn} with varying tuning parameters, we obtain the results depicted in \Cref{tab:results.knn} which contains a selection of the best models.\footnote{The complete logs can be found alongside the project's code at \url{https://github.com/fontaine618/507-Project/}.} 
...
\input{./table/nn_features.tex}


%------------------------------------------------------------------------------
\subsection{Matrix completion}\label{sebsec:results.svd}
We trained around 100 instances of matrix completion model as describes in \Cref{subsubsec:method.models.svd}. We choose to present some models with its parameters. From the result, we can see some facts about turning parameters as below.
...
We trained around 100 instances of RBM model as describes in \Cref{subsubsec:method.models.rbm}. We choose to present some models with its parameters. From the result, we can see some facts about turning parameters as below.
